...
class UserService(Service):
    ...
    async def check_tokens_and_time(
        ...
        """Проверяет минимальное время между запросами."""
        now = datetime.now()
    ...
    async def edge_voice_generate(
        ...
        """Генерирует голосовое сообщение используя Edge TTS."""
        self.logger.info(f"Попытка записи голосового сообщения для {user_id}")
        ...
        try:
            ...
            if voice_bytes:
                self.logger.info(f"Edge TTS (библиотека): {len(voice_bytes)} байт")
        except Exception as e:
        ...
        if not voice_bytes:
            ...
            if edge_tts_url and edge_tts_url != "" and edge_tts_url != "your_edge_tts_api_url":
                    ...
                    async with aiohttp.ClientSession() as session:
                        async with session.post(
                            ...
                            if response.status != 200:
                                ...
                                if voice_bytes:
                                    self.logger.info(f"Edge TTS (API): {len(voice_bytes)} байт")
                                else:
                ...
                self.logger.debug("Edge TTS API URL не указан, пропускаем")
    ...
    async def apply_voice_effect(
        ...
        """Применяет эффекты к голосовому сообщению. Поддерживает форматы MP3, WAV, OGG. Конвертирует в OGG для Telegram."""
            ...
            if audio_format == "mp3":
                try:
                    ...
                    try:
                        ...
                        out_buffer.seek(0)
                        self.logger.info("Эффект для голосового был применен успешно (librosa)")
                ...
                try:
                    ...
                    sf.write(out_buffer, processed, samplerate, format='OGG', subtype='VORBIS')
                    out_buffer.seek(0)
                    self.logger.info("Эффект для голосового был применен успешно")
                    return out_buffer.read()
